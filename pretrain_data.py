...
def load_and_extract_tokens(csv_path="resource/automation_log10.csv"):
    ...
    # Create a token: "url | simplified_selector | action"
    df["token"] = df["url"].str.strip() + " | " + df["selector"] + " | " + df["action"]
...
def train_lstm_model(X_train, y_train, vocab_size, input_length, model_path="resource/lstm_model.keras"):
    model = Sequential([
...
def main():
    ...
    # Train the model
    model = train_lstm_model(X_train, y_train, vocab_size, input_length, model_path="resource/lstm_model.keras")
    print("Model trained and saved as resource/lstm_model.keras")
